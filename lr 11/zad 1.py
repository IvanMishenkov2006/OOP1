class Person:
    ...
    def __str__(self):
        return f'Имя и фамилия: {self.name}, Возраст: {self.age}'  # Возвращает строку с информацией о человеке.
...
class Worker(Person):  # Наследуется от класса Person.
    ...
    def __str__(self):
        return f'Имя и фамилия: {self.name}, Возраст: {self.age}, Работа: {self.profession}'  # Возвращает строку с информацией о работнике.
...
class Father(Person):  # Наследуется от класса Person.
    ...
    def __str__(self):
        return f'Имя и фамилия: {self.name}, Возраст: {self.age}, Кол. детей: {self.children}'  # Возвращает строку с информацией о отце.
...
class WorkingFather(Worker, Father):  # Наследуется от классов Worker и Father.
    ...
    def __str__(self):
        return f'Имя и фамилия: {self.name}, Возраст: {self.age}, Работа: {self.profession}, Кол. детей: {self.children}'  # Возвращает строку с информацией о работающем отце.


person = Person('Артур Пряников', 46)
print(person)  # Выводит информацию о person.

worker = Worker('Виталик Смит', 56, 'Актер')
print(worker)  # Выводит информацию о worker.

father = Father('Михаил Джордж', 34, 4)
print(father)  # Выводит информацию о father.

working_father = WorkingFather('Саня Козырный', 29, 'Продавец(в прошлом)', 0)
